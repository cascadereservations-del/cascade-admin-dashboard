...
| 3 — Backend privacy | Reviewable SQL draft for audit-feed access, merge scope/concurrency, profile validation and storage scope | Isolated Cascade rehearsal, current production definition diff, verified backup, full role/property tests. Do not use Alfred/Alex or any unrelated database as rehearsal. |
| 4 — Private Drive handoff | Exact destination confirmed; sharing restricted; automation dependency checked; organized uploads and local receipts | User resolves sharing after Telegram compatibility check. Never send IDs to Telegram or other destinations as a test. Verify every uploaded file’s parent, size and checksum where available. |
| 5 — Review and rollout | Versioned build, owner approval to push, operator smoke test | Publish only reviewed code/build artifacts, no private manifest. Canary entry point first; observe before updating root. |
